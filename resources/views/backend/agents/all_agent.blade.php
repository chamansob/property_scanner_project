                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                ...
                                                <form action="{{ route('admin.agent_delete', $agent->id) }}"
                                                    ...
                                                    @csrf
                                                    @method('DELETE')
                                                <a href="{{ $agent->status == 1 ? 'javascript:void(0)' : url('agent/details/' . $agent->id . '/' ) }} "
                                                        class="btn btn-inverse-info" title="Email Varification" {{ $agent->status == 1 ? '' : 'target="_blank"' }}  > <i
                                                            data-feather="eye"></i> </a>
                                                            <a href="{{ $agent->email_verified_at != '' ? 'javascript:void(0)' : route('admin.agent_email_varification',$agent->id)  }} "
                                                        class="btn btn-inverse-{{ ($agent->email_verified_at != '') ? "light" : "danger"  }}" title="Agent View"  > <i
                                                        {{ ($agent->email_verified_at != '') ? "data-feather=check" : "data-feather=x"  }} 
                                                           ></i> </a>
                                                    <a href="{{ route('admin.agent_edit', $agent->id) }}"
                                                        class="btn btn-inverse-warning"><i data-feather="edit"></i> </a>
                                                    <a href="{{ route('admin.agentplan.add', $agent->id) }}"
                                                        class="btn btn-inverse-warning"><i data-feather="plus"></i> </a>
